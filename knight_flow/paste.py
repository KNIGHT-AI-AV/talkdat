...
def strip_messenger_trailing_period(text: str) -> str:
    ...
    **It is off by default and should stay off until it has been tried in the
    ...
    with _EXTERNAL_DELIVERY_LOCK:
        ...
        try:
            ...
            if not _delivery_is_allowed(can_deliver) or not copy_text(replacement):
                return UndoReplacementReceipt(False, False)
            if not _delivery_is_allowed(can_deliver):
                return UndoReplacementReceipt(False, False)
            pyautogui.PAUSE = 0
...
def paste_text_with_receipt(
    ...
    *,
    ...
    if text:
        text = strip_messenger_trailing_period(text)
    ...
    settle_modifiers()
    copied = bool(text and _delivery_is_allowed(can_deliver) and copy_text(text))
